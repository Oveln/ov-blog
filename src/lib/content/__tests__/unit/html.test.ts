...
describe("renderHtml", () => {
  ...
  it("渲染行内代码", async () => {
    const html = await renderHtml("Use `npm install` to install")
    expect(html).toContain("<code>")
    expect(html).toContain("npm install")
  })

  it("渲染代码块并高亮", async () => {
    const html = await renderHtml("```typescript\nconst x = 1\n```")
  ...
  it("渲染 GFM 任务列表", async () => {
    const html = await renderHtml("- [x] done\n- [ ] todo")
    expect(html).toContain("checkbox")
    expect(html).toContain("checked")
  })

  it("渲染数学公式为 KaTeX", async () => {
    const html = await renderHtml("$E = mc^2$")
  ...
  it("渲染粗体和斜体", async () => {
    const html = await renderHtml("**bold** and *italic*")
    expect(html).toContain("<strong>bold</strong>")
    expect(html).toContain("<em>italic</em>")
  })

  it("渲染链接", async () => {
    const html = await renderHtml("[link](https://example.com)")
    expect(html).toContain('href="https://example.com"')
    expect(html).toContain(">link<")
  })

  it("渲染图片", async () => {
    const html = await renderHtml("![alt](https://example.com/img.png)")
  ...
  it("渲染分割线", async () => {
    const html = await renderHtml("above\n\n---\n\nbelow")
    expect(html).toContain("<hr")
  })

  it("中文内容渲染正常", async () => {
    const html = await renderHtml("## 你好世界\n\n这是一段中文内容。")

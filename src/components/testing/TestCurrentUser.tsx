...
export function TestCurrentUser() {
  const { data: currentUser, isLoading: userLoading, error: userError } = trpc.user.getCurrentUser.useQuery();
  const { data: username, isLoading: nameLoading, error: nameError } = trpc.user.getUsername.useQuery();
  ...
  return (
    <div className="p-4 border rounded-lg bg-gray-50">
      ...
      <div className="space-y-2">
        <div>
          <span className="font-medium">User ID:</span> {currentUser.id ?? 'No ID'}
        </div>
        <div>
          <span className="font-medium">Name:</span> {currentUser.name ?? 'Not set'}
        </div>
        <div>
          <span className="font-medium">Email:</span> {currentUser.email ?? 'No email'}
        </div>
        <div>
